...
class TestPayloadRun:
    ...
    def fix_payload(self, project_config, fix_plugin_config) -> S3PParserBase:
        MODULE_NAME: str = 's3p_plugin_parser_ieee'
    ...
    def run_payload(self, payload: Union[S3PParserBase, Callable], refer: S3PRefer, _plugin: S3PPlugin, restrictions: S3PPluginRestrictions, driver: WebDriver, params):

        _params = {}
        for param in params:
            if param.typeof == 'const':
    ...
    def test_all_cases_with_once_executing_parser(self, chrome_driver, fix_s3pRefer, fix_payload, fix_s3pPlugin, fix_plugin_config):
        """
        Test Case
        ...
        """
        max_docs = 4
        docs = self.run_payload(fix_payload, fix_s3pRefer, fix_s3pPlugin, S3PPluginRestrictions(max_docs, None, None, None), chrome_driver, fix_plugin_config.payload.entry.params)
